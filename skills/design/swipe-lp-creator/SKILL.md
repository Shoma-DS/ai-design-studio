...
## Step 0.5: 原稿設計の鉄則（外部記事の知見を反映、2026-07-16／2026-07-19追記）

[アナグラム社の検証記事](https://anagrams.jp/blog/swipeable-lp-benefits-and-results/)、[Letro社の解説記事](https://service.aainc.co.jp/product/letro/article/landing-page-swipetype)、[Hatch社の制作ガイド](https://lab.tryhatch.co.jp/swipe-lp/swipe-lp-production/)、[yomyom(ColorMe)の記事](https://shop-pro.jp/yomyom-colorme/106717)、[digitalidentity社の記事](https://digitalidentity.co.jp/blog/generative-ai/swipe-lp.html)から、以下をスワイプ型LP制作の原則として適用する。
...
- **心情がピークに達するのは解決策提示の直後**: ユーザーの行動意欲は「解決策（solution/features系カード）」を見た直後に最も高まる。情報訴求型でsolutionカードから最終CTAカードまでの間に証拠系カード（実績・お客様の声・代表挨拶など）を挟みすぎると、このピークが冷める。証拠系カードは必要最小限に絞り、solutionから最終オファーまでの距離を空けすぎない。常時表示CTAバー（Step 4）は全カード共通で最初から出ているため、このルールは「バーをどこに置くか」ではなく「盛り上がりを冷まさないカード順」の話として適用する。
- **CTA文言は行動を具体化する**: 「詳しくはこちら」のような抽象的な文言より、「無料で試してみる」「今すぐ相談する」のように具体的な行動をイメージさせる文言の方が効果的。`copy/sections/<section-id>.md` の`cta`欄・固定CTAバーのラベルとも、この基準で書く。
...
- EC・アパレル・コスメ・食品・インテリアなど、ビジュアルそのものが訴求力になる商材
- SNS広告（Instagram/TikTok）が主流入経路になる商材（カルーセル広告と操作感が地続きになりCVに寄与する）

逆に、以下に当てはまる商材は無理にスワイプ型を勧める必要はない（縦長LPで十分）。

- 説明に必要な情報量がそもそも少ない商材
- 公開後の分析・改善を回す体制を確保できない案件
- **BtoB高額商材で、仕様比較や情報量の多さが検討の決め手になるもの**: スワイプ型は構造上1カードあたりの情報量を絞る必要があり、比較検討に必要な詳細情報を出し切れない。自然検索やメルマガなど「能動的に詳細情報を求める」流入経路が主体の商材とも相性が悪い（スワイプ型は広告流入前提の設計で、SEO流入には不向き）。
...
- **企画段階で数値目標を具体化する**: 計測項目を並べる前に「誰に」「どんな行動を」「いつまでに何件」を先に決める（例:「月間受注件数◯件」）。目標が曖昧なままだと、後段のカード単位計測・A/Bテストも何を改善指標にすべきか定まらない。
- カード到達率、CTAクリック率、滞在時間などカード単位の計測（GA4のイベント設計等）
- 「本当に見るべきKPI」が手前のCVだけでなく、その先（本申込・入金など）にある商材かどうかの確認
- A/Bテスト前提で、後から差し替えやすいカード単位の構造で作る（この点は元々Step 4の実装方針と整合する）。テスト対象の例: ファーストビュー（フックカード）の画像、CTAボタンの文言・色、カードの並び順。

## Step 1: 架空ブランド・原稿（セクションごとに個別ファイル化する）

`skills/design/lp-creator` と同じ注意点に従う（実在企業・商品・ロゴ・コピーの模写は禁止、完全オリジナルの架空ブランド）。AGENTS.mdの「原稿、セクション別画像プロンプト、生成画像、結合LPを分けて保存する」に従い、以下の3段階を**この順番で**進める。1段階目・2段階目とも1ファイルにまとめず、必ずカードごとに個別ファイルにする。

1. **全体構成**: `portfolio/<slug>/copy/構成.md` に、ブランド設定・参考分析・カード構成の全体像（一覧表）を作る。
2. **セクションごとのLP原稿作成**: 各カードのHTMLオーバーレイに使う文言（eyebrow／見出し／本文／CTAラベルなど）を `portfolio/<slug>/copy/sections/<section-id>.md` として**1カード1ファイル**で作る。構成.mdの一覧表はあくまで全体像の要約とし、実際に実装で使う確定文言はこのファイルを正とする。ファイル内フォーマット:
   ```markdown
   # <section-id>（カードの役割）

   - eyebrow: （小見出し・ラベル等、なければ省略）
   - headline: （メインコピー）
   - body: （本文、箇条書き・番号リストが必要ならここに列挙）
   - cta: （このカード固有のCTA文言があれば。共通CTAバーのみの場合は省略）
   ```
3. **セクションごとの画像プロンプト作成**: 各カードの画像プロンプトは `portfolio/<slug>/copy/prompts/<section-id>.md` として**1カード1ファイル**で作る。ファイル内フォーマット:
   ```markdown
   # <section-id>（カードの役割）
   ...
   （実際のプロンプト本文）
   ```
   `section-id` は生成する画像ファイル名（`01-hook` など）と一致させ、`copy/sections/`・`copy/prompts/`・画像・HTML・スクリプトを横断して同じ名前で追跡できるようにする。
...
portfolio/<slug>/
  copy/構成.md
  copy/sections/<section-id>.md  # カードごとのLP原稿・確定文言（Step 1-2）
  copy/prompts/<section-id>.md   # カードごとの画像プロンプト（Step 1-3）
  lp/images/                     # 静止画（全カード共通のフォールバック/posterも含む、Step 3でラスター生成）
...
## Step 3: 画像生成（プロンプトファイルから読み込み、ラスター画像として生成する・重要な落とし穴）
...
- **公開前にWeb配信用へ最適化する**: 生成直後のPNGはそのまま置かず、WebP形式に変換し1枚あたり100〜200KB程度に圧縮する（表示速度が3秒を超えると離脱率が急増するというデータがあるため）。幅は生成時の1080pxのままでよい（推奨レンジ750〜1080pxの上限に収まる）。`.card-bg` の `<img>` には元PNG/JPGをフォールバックとして残しつつ `<picture>` + `<source type="image/webp">` で配信するか、変換後のWebPをそのまま `src` に使う。
...
- **常時表示CTAバー**: 最終カードだけにCTAボタンを置くと、そこまでスワイプしないと申込導線が見えない。`.carousel` 直下に `position: absolute; bottom: 0;` の固定バー（`.sticky-cta`）を置き、全カード共通で表示する。最終カード自体には重複ボタンを置かず固定バーに一本化し、タップ時は `goTo(cards.length - 1)` で最終オファーカードへジャンプさせる。**外部記事には「重要スライド下部にも文脈CTAを複数配置する」という別方式もあるが、このスキルでは常時バー一本化を標準とする**（1本化の方が実装がシンプルで、全カード共通で申込導線が常に見えるという狙いを既に満たしているため）。カードごとに個別CTAを追加したいという明示の要望があった場合のみ検討する。
- **インジケーターの位置**: 固定CTAバーと重ならないよう、`.indicator` の `bottom` をCTAバーの高さ分（目安94px）上げる。
- **全カードの高さを統一する**: `.card`/`.h-card` は静止画・動画・情報量の多い浮き島カードのどれであっても、必ず同じ高さ（`.carousel` の `height: 100dvh` 等を継承）にする。カードごとに高さが変わると、スワイプ時に画面が微妙にガタつきユーザーがストレスを感じて離脱しやすくなる。特に動画カード（`<video>`）と静止画カード（`<img>`）が混在する章や、`card--group` の章内カード（`.h-card`）で高さがずれやすいので、`object-fit: cover` と固定height指定を全カード共通で徹底する。
- **画像・動画にalt/aria-labelを付ける**: このLP構成は情報を写真に焼き込まず文字はHTMLオーバーレイで持つ設計だが、`.card-bg` の `<img>`/`<video>` 自体には視覚的な内容を説明するalt属性（`<video>`の場合はaria-label）を付ける。画像主体のスワイプLPは検索エンジンに内容が伝わりにくいという弱点があり、alt/aria-labelが最低限のSEO対応になる。
