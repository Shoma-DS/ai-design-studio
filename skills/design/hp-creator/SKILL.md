...
対象URLをclaude-in-chromeで開き、以下を調べる。**下層ページ（会社概要・料金・事例・お問い合わせ等ナビにあるページ）も含めて全ページを1ページずつ開き、フル幅スクリーンショットを撮る**。トップページだけ見て「だいたいこんな感じ」で下層ページを想像で作らない。1ページ確認→スクショ→分析→（後続手順で）実装→比較検証、を全ページぶん繰り返す。

1. **セクション構成**：ページ全体をスクロールしながら、各セクションの役割・順番をメモする。
2. **ファーストビュー／各セクションのレイアウト実測（重要）**：言葉で「〜がある」とメモするだけでなく、要素の配置順・比率・具体的な装飾効果までスクリーンショットを見ながら書き出す。例：「見出しは画面幅の90%、太字＋オフセットした薄色の二重シャドウで立体的に見せている」「見出し下は2カラムで、左＝紹介文＋ブランド名の大型ロゴ表示、右＝訴求コピー＋破線＋強調ハイライト＋画面キャプチャ枠」「CTAボタンの下にカルーセルドットインジケーターがある」など。ここが浅いと、実装後に「なんとなく雰囲気は近いが全然似ていない」状態になる。
...
8. **著作権配慮（重要）**：再現してよい範囲と避けるべき範囲を明確に切り分ける。
   - **忠実に再現してよい**：レイアウト構造、要素の配置順・比率、装飾効果（影・下線・ハイライト・ドットインジケーター・カード形状など汎用的なUI表現）、ページ構成の型、セクションの遷移の仕方。
   - **避けるべき（コピー・トレース禁止）**：参考サイトの実写真・ロゴ・商標・コピー文言そのままの転記、配色をそのまま流用すること。
   - 「構造だけ参考にする」を「レイアウトはざっくり似せて中身は変える」と誤読しない。配色・コピー・ロゴ・実写真だけ差し替えれば、レイアウト自体はプロダクト品質を優先してかなり忠実に再現してよい。

### 1.5 実装直後の忠実度検証（各主要セクションごとに必須）

ヒーローなど主要セクションを実装したら、そのままフルサイト実装を進めず、**その場で**参考サイトのスクリーンショットと自分の実装のスクリーンショットを並べて見比べる。要素の配置順・比率・装飾効果に具体的なズレがないか確認し、ズレがあれば先に直してから次のセクションに進む。全ページ実装後にまとめて比較すると手戻りが大きくなるため、セクション単位でこまめに検証する。
...
- **お問い合わせ／申し込みフォームがある場合**、フォーム直前（フォームのすぐ上）に実績・取引先・導入企業のロゴを横並びで置くセクションを企画に含める。コンバージョン直前は「この会社、信用できる？」という不安から離脱しやすいタイミングで、実績ロゴを置くだけでCVRが改善する事例がある（[アプリマーケティング研究所の投稿](https://x.com/appmarkelabo/status/2081695822731981258)、実例ではCVR約12%改善）。架空企業のポートフォリオ案件ではロゴも架空のダミー企業名で作る。クライアント案件では実在の実績先ロゴを使う許諾があるかを確認し、無ければ「導入実績◯◯件」等の数値やお客様の声で代替する。
