...
────────────────
【このセクション固有のPC構図指示（最優先）】
・スマホ版・PC版とも同じ2枚の写真（左写真・右写真）を使うが、スマホ版のような単純な50:50の左右均等分割をそのまま横に引き伸ばさない。
・PC版では左右の比率を約60:40など非対称にし、さらに一方の写真パネルをもう一方より上下にわずかにオフセットして高さをずらすことで、スマホ版とは異なる列配置比率にする（写真の枚数・内容・トリミング対象は変えない）。
`,
  "06-purchase-method": `
────────────────
【このセクション固有のPC構図指示（最優先）】
