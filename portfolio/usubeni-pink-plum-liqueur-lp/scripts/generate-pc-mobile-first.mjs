...
const SECTION_SPECIFIC_INSTRUCTION = {
  "02-intro-price": `
────────────────
【このセクション固有のPC構図指示（最優先・厳守。過去に「中央揃えの単一カラムのまま価格と予約期間だけを横並びにした」構図に戻ってしまった実績があるため、以下を数値レベルで厳密に守ること）】

これまでの生成結果は、見出し帯・商品名を上部中央に大きく配置し、その下に価格と予約期間だけを横並びにする「実質は中央揃えの単一カラム」構図だった。**その構図は禁止。今回はセクション全体を左右2カラムに分割する。**

■ 全体を縦の中心線で左右2つのカラムに分割する（左右カラムの境界に細い縦の区切り線を入れてよい）。中央揃えで要素を積み重ねる構成には戻さない。

■ 左カラム（横幅の約50〜55%、テキストは左揃えまたはカラム内中央揃え）:
  - 小見出し帯「数量限定 先行予約受付中」
  - 区切り線（横棒、小見出しの下）
  - 商品名（大きめ・明朝体）「薄紅 the deep / 薄紅 とろみ 飲み比べセット」
  左カラムはこの3要素だけを縦に積み、価格・予約期間・注記は置かない。

■ 右カラム（横幅の約45〜50%、独立した情報ボックスとして視覚的にまとまりを持たせる。囲み枠・淡い背景色パネル・左端の縦の飾り線などでカード状に見せてよい）:
  - 価格「5,980円（税込）」
  - 予約期間「7月10日（金）18時 〜 7月24日（金）12時」
  - 注記「予約の時点では料金は発生しません。」
  この3要素を右カラム内に縦に積む（横一列には並べない）。

■ 左右カラムは同じ高さ付近に上端を揃え、セクション全体は横長の帯として1200px幅のコンテナに収まるようにする。

■ 禁止事項（厳守）:
・商品名・見出し帯・価格・予約期間・注記をすべて中央揃えで縦一列に積むこと（スマホ版のレイアウトをそのまま横に引き伸ばしただけの構図）
・価格と予約期間だけを横並びにし、それ以外（見出し帯・商品名）は中央上部に据え置く構図
・要素を左右に散らしただけで、見出しか商品名を取り除くとスマホ版とほぼ同じシルエットに戻ってしまうような配置
`,
  "03-mood-photo": `
────────────────
