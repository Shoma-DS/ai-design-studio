...
async function generateOne(section) {
  ...
  const refName = section.id === "01-hero" || section.id === "02-message"
    ? "reference-tone-hero.png"
    : "reference-tone.png";
  const refBuffer = await fs.readFile(path.join(refDir, refName));
  ...
  const generated = await generateImageWithCodexAppServer({
    ...
    refImages: [{ name: `${section.id}-tone-reference.png`, buffer: refBuffer }],
    cwd: repoRoot,
    taskType: "showcase",
  });
...
if (failures.length) {
  console.error(`${failures.length} mobile image generation tasks failed: ${failures.map((f) => f.id).join(", ")}`);
  process.exitCode = 1;
} else {
