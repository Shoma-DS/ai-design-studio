...
async function main() {
  const prompt = await fs.readFile(path.join(OUT_DIR, "prompt-edit-remove-woman.md"), "utf-8");
  await fs.mkdir(OUT_DIR, { recursive: true });

  const refImages = [
    { path: path.join(OUT_DIR, "01-hero-no-badge.png"), name: "current-hero.png" },
  ];

  console.log("Generating hero background with woman removed...");
  const result = await generateImageWithCodexAppServer({
    prompt,
    sectionId: "belle-rouge-hero-remove-woman",
    imageName: "01-hero-no-woman.png",
    refImages,
    cwd: REPO_ROOT,
    taskType: "showcase",
  });
  ...
  const outPath = path.join(OUT_DIR, "01-hero-no-woman.png");
