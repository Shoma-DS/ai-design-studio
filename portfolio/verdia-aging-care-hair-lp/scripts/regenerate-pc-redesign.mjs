...
・カードの下の注釈と「HOME CARE LINE UP」見出しはカード下部に中央揃えで残してよい。`,
  "10-system-table": `
・比較表はPCの横幅を活かし、列数を増やして構成する。「アウトバストリートメント」の行はGRAND SILK列・GRAND SATIN列それぞれに個別セルを分けて配置する（スマホ版のような2列またぎの単一セルにしない）。
・各セルは横幅に余裕を持たせ、製品写真とライン名を横並びまたは余裕のある配置にし、表全体がスマホ版よりも横に広いグリッドに見えるようにする。
・左端の「ホームケア」縦見出しは維持してよいが、表本体は縦に間延びさせず、横方向の情報量を増やす構成にする。`,
};
