...
async function main() {
  ...
  const prompt = await fs.readFile(path.join(PROJECT_DIR, "prompt-line-richmenu.md"), "utf-8");
  console.log("Generating line-richmenu.png...");
  const result = await generateImageWithCodexAppServer({
    prompt,
    sectionId: "lancers-sample-line-richmenu",
    imageName: "line-richmenu.png",
    refImages: [],
    cwd: REPO_ROOT,
    taskType: "showcase",
  });

  const outPath = path.join(OUT_DIR, "line-richmenu.png");
