...
function buildImageGenerationPrompt(prompt, sectionId, imageName, refImageNames = [], taskType = "section") {
  ...
  return [
    ...
    "Use Codex app-server's built-in image_generation capability exactly once.",
    "Do not use shell commands, filesystem writes, web search, external APIs, OpenAI API keys, or metered image APIs.",
    "Create one polished landing-page section image for PC/desktop display, sized for a max-width 1200px centered desktop LP section (the image will be scaled to fit up to 1200px width, height auto, and stacked with other sections).",
    "Base the composition on the mobile version's information design (same content hierarchy, icon/illustration style, brand feel) but restructure the element layout to use the desktop width: elements stacked vertically on mobile should typically become side-by-side columns, and mobile grids/lists should typically become wider horizontal rows. Do not simply stretch or scale up the mobile layout sideways into a shallow banner with the same relative positions.",
    "Target a roughly 16:9 landscape aspect ratio (about 1536x1024) as the default frame for this PC section. Only deviate toward a taller frame when the section's content genuinely cannot be reflowed into a landscape frame.",
    "The result should feel like a natural, well-proportioned desktop evolution of the mobile design with a clearly different element arrangement, not a different layout language and not a same-arrangement resize.",
