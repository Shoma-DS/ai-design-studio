...
> **Adaptation note for ai-design-studio:** this skill is copied verbatim from [JCodesMore/ai-website-cloner-template](https://github.com/JCodesMore/ai-website-cloner-template), which assumes a Next.js + shadcn/ui + Tailwind v4 scaffold (`src/app/layout.tsx`, `npm run build`, etc.). This repo does not use that stack — LP/HP work here is static HTML/CSS/JS or CSS Modules (see `skills/design/lp-creator/`, `skills/design/hp-creator/`). Before running this skill against a target site, either (a) treat it as extraction-only and hand the resulting `docs/research/` specs to the studio's own LP/HP builder skills instead of the Next.js builder-agent/worktree pipeline below, or (b) confirm with the user that this specific task should spin up a separate Next.js scaffold. Do not silently `npm run build` against this repo's existing projects.
