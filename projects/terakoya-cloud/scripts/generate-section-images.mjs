...
const projectRoot = path.join(repoRoot, "projects/terakoya-cloud");
...
const concurrency = Math.max(1, Math.min(Number(process.env.LP_IMAGE_CONCURRENCY || 1), 4));
...
async function generateOne(section) {
  ...
  if (!generated.configured) {
    throw new Error(generated.message || "Codex app-server の画像生成が設定されていません。");
  }
