...
const PROJECT_DIR = path.join(REPO_ROOT, "projects/coconala-blog-lp-order-thumbnail");
...
async function main() {
  ...
  console.log("Generating coconala blog cover thumbnail...");
  const result = await generateImageWithCodexAppServer({
    prompt,
    sectionId: "coconala-blog-lp-order-thumbnail",
