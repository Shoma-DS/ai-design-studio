...
async function main() {
  ...
  const refImages = [
    { path: path.join(OUT_DIR, "thumbnail-v3-headline.png"), name: "current-thumbnail.png" },
  ];

  console.log("Generating v4 (6:5 safe-margin resize) ...");
  const result = await generateImageWithCodexAppServer({
    prompt,
    sectionId: "coconala-lp-thumbnail-v4-safe-margin",
    imageName: "thumbnail-v4-safe-margin.png",
    refImages,
    cwd: REPO_ROOT,
    taskType: "showcase",
  });

  const outPath = path.join(OUT_DIR, "thumbnail-v4-safe-margin.png");
