...
async function main() {
  const prompt = await fs.readFile(path.join(PROJECT_DIR, "prompt-edit-add-topband.md"), "utf-8");
  await fs.mkdir(OUT_DIR, { recursive: true });

  const refImages = [
    { path: path.join(OUT_DIR, "thumbnail-v4-safe-margin.png"), name: "img1.png" },
  ];

  console.log("Generating coconala-lp-thumbnail edit: add topband...");
  const result = await generateImageWithCodexAppServer({
    prompt,
    sectionId: "coconala-lp-thumbnail",
    imageName: "thumbnail-v5-add-topband.png",
    refImages,
    cwd: REPO_ROOT,
    taskType: "showcase",
  });

  const outPath = path.join(OUT_DIR, "thumbnail-v5-add-topband.png");
