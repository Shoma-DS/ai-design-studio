...
async function main() {
  const prompt = await fs.readFile(path.join(PROJECT_DIR, "prompt-edit-fix-garbled-text.md"), "utf-8");
  await fs.mkdir(OUT_DIR, { recursive: true });

  const refImages = [
    { path: path.join(OUT_DIR, "thumbnail-v3-faithful-reproduction.png"), name: "img1.png" },
  ];

  console.log("Editing coconala animated-LP thumbnail: fix garbled text...");
  const result = await generateImageWithCodexAppServer({
    prompt,
    sectionId: "coconala-animated-lp-thumbnail",
    imageName: "thumbnail-v4-fix-garbled-text.png",
    refImages,
    cwd: REPO_ROOT,
    taskType: "showcase",
  });

  const outPath = path.join(OUT_DIR, "thumbnail-v4-fix-garbled-text.png");
