...
async function main() {
  const prompt = await fs.readFile(path.join(PROJECT_DIR, "prompt-edit-badge-remove-box.md"), "utf-8");
  await fs.mkdir(OUT_DIR, { recursive: true });

  const refImages = [
    { path: path.join(OUT_DIR, "thumbnail-v10-badge-style.png"), name: "img1.png" },
  ];

  console.log("Editing coconala animated-LP thumbnail: remove badge box and × from 集客UP...");
  const result = await generateImageWithCodexAppServer({
    prompt,
    sectionId: "coconala-animated-lp-thumbnail",
    imageName: "thumbnail-v11-badge-remove-box.png",
    refImages,
    cwd: REPO_ROOT,
    taskType: "showcase",
  });

  const outPath = path.join(OUT_DIR, "thumbnail-v11-badge-remove-box.png");
