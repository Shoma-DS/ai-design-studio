...
async function main() {
  const prompt = await fs.readFile(path.join(PROJECT_DIR, "prompt-v2-red-redesign.md"), "utf-8");
  await fs.mkdir(OUT_DIR, { recursive: true });

  const refImages = [
    { path: path.join(PROJECT_DIR, "references/ref-red-layout.jpeg"), name: "ref-red-layout.jpeg" },
  ];

  console.log("Generating coconala thumbnail red redesign...");
  const result = await generateImageWithCodexAppServer({
    prompt,
    sectionId: "coconala-animated-lp-thumbnail",
    imageName: "thumbnail-v12-red-redesign.png",
    refImages,
    cwd: REPO_ROOT,
    taskType: "showcase",
  });

  const outPath = path.join(OUT_DIR, "thumbnail-v12-red-redesign.png");
