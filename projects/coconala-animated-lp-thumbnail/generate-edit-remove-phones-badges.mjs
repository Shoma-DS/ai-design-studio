...
async function main() {
  const prompt = await fs.readFile(path.join(PROJECT_DIR, "prompt-edit-remove-phones-badges.md"), "utf-8");
  ...
  console.log("Editing coconala animated-LP thumbnail: remove phone mockups and orange badges...");
  const result = await generateImageWithCodexAppServer({
    prompt,
    sectionId: "coconala-animated-lp-thumbnail",
    imageName: "thumbnail-v9-remove-phones-badges.png",
    refImages,
    cwd: REPO_ROOT,
    taskType: "showcase",
  });

  const outPath = path.join(OUT_DIR, "thumbnail-v9-remove-phones-badges.png");
