...
async function main() {
  const prompt = await fs.readFile(path.join(PROJECT_DIR, "prompt-edit-shrink-headline.md"), "utf-8");
  await fs.mkdir(OUT_DIR, { recursive: true });

  const refImages = [
    { path: path.join(OUT_DIR, "thumbnail.png"), name: "img1.png" },
  ];

  console.log("Editing coconala animated-LP thumbnail: shrink headline...");
  const result = await generateImageWithCodexAppServer({
    prompt,
    sectionId: "coconala-animated-lp-thumbnail",
    imageName: "thumbnail-v2-shrink-headline.png",
    refImages,
    cwd: REPO_ROOT,
    taskType: "showcase",
  });

  const outPath = path.join(OUT_DIR, "thumbnail-v2-shrink-headline.png");
